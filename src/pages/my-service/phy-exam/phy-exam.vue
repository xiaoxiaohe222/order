<template>
    <view class="order">
        <view v-if="coviduserOrder.length" class="card" v-for="(item, index) in coviduserOrder" :key="index">
            <view class="address">体检套餐:{{item.phy_name}}</view>
            <view class="address">体检人:{{item.name}}</view>
            <view class="company">体检时间: {{item.time}}</view>
            <view class="company">体检地点: {{item.address}}</view>
...
import { IPhyUserOrder } from '../../../my-type'
import { reactive, toRefs } from "vue";
import NoData from "@/components/NoData.vue";
const state = reactive({
    coviduserOrder: [] as IPhyUserOrder[]
})
onShow(() => {
    getData()
})
// 获取订单数据
const getData = async () => {
    const res: any = await request('/phyuser_order', {}, 'GET')
    if (res.msg === 'SUCCESS') {
        state.coviduserOrder = res.data
    }
}
// 取消预约
const cancel = async (item: IPhyUserOrder) => {
