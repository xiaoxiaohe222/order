<template>
    <view class="order">
        <view v-if="coviduserOrder.length" class="card" v-for="(item, index) in coviduserOrder" :key="index">
            <view class="name">{{item.hpv_name}}</view>
            <view class="address">接种人:{{item.name}}</view>
            <view class="address">接种地点:{{item.address}}</view>
            <view class="company">疫苗套餐: {{item.combo}}</view>
...
import { IHpvUserOrder } from '../../../my-type'
import { reactive, toRefs } from "vue";
import NoData from "@/components/NoData.vue";
const state = reactive({
    coviduserOrder: [] as IHpvUserOrder[]
})
onShow(() => {
    getData()
})
// 获取订单数据
const getData = async () => {
    const res: any = await request('/hpvuser_order', {}, 'GET')
    if (res.msg === 'SUCCESS') {
        state.coviduserOrder = res.data
    }
}
// 取消预约
const cancel = async (item: IHpvUserOrder) => {
    const res:any = await request('/hpvcancel?_id=' + item._id, {}, 'GET')
    if (res.msg === 'SUCCESS') {
